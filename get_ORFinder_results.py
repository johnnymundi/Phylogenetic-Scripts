...
def orffinder_search(list):
    ...
    for tag in range(len(sequences)):
        print('sequencia numero: ' + str(tag + 1) + '' + str(tags[tag]))
        ...
        sense = driver.find_element_by_xpath(
            '/html/body/div[1]/div[1]/div[4]/div/form/fieldset[2]/div[3]/ul/li[3]/label/input')
        sense.click()

        # clica no botão Submit para ver o resultado da busca
        button = driver.find_element_by_id('button_submit')
        button.click()
        time.sleep(15)  # espera um tempo para aparecer o resultado
